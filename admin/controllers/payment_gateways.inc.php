...
/* ext js action end */
    ...
    if (isset($_GET['action'])) {
        ...
        if ($gatewayNotice = $lang[$gateway_info['Key'] . '_notice']) {
            $reefless->loadClass('Payment');

            // Replace ".html" to "/" if the ".html" usage is off
            if (!$config['html_in_pages']
                && (
                    false != strpos($gatewayNotice, '/' . $rlPayment::POST_URL . '.html')
                    || false != strpos($gatewayNotice, '/' . $rlPayment::CHECKOUT_URL . '.html')
                    || false != strpos($gatewayNotice, '/' . $rlPayment::SUCCESS_URL . '.html')
                    || false != strpos($gatewayNotice, '/' . $rlPayment::FAIL_URL . '.html')
                )
            ) {
                $gatewayNotice = str_replace(
                    [
                        '/' . $rlPayment::POST_URL . '.html',
                        '/' . $rlPayment::CHECKOUT_URL . '.html',
                        '/' . $rlPayment::SUCCESS_URL . '.html',
                        '/' . $rlPayment::FAIL_URL . '.html',
                    ],
                    [
                        '/' . $rlPayment::POST_URL . '/',
                        '/' . $rlPayment::CHECKOUT_URL . '/',
                        '/' . $rlPayment::SUCCESS_URL . '/',
                        '/' . $rlPayment::FAIL_URL . '/',
                    ],
                    $gatewayNotice
                );
            ...
            $rlNotice->saveNotice($gatewayNotice, 'alerts');
        }

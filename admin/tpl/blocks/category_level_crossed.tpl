    ...
    <ul {if $first}class="first"{/if}>
        {foreach from=$categories item='cat' name='catF'}
            {if !empty($cat.Sub_cat) || ($cat.Add == '1' && $listing_types[$cat_type].Cat_custom_adding)}
                {assign var='sub_leval' value=true}{else}{assign var='sub_leval' value=false}
            {/if}
            <li id="tree_cat_{$cat.ID}" {if $cat.Lock}class="locked"{/if}>
                ...
                <label>
                    <input {if $cat.Lock || $cat.ID == $category_id}disabled="disabled" class="system"{/if}
                        type="checkbox"
                        name="crossed_categories[]"
                        value="{$cat.ID}" />
                    <span>{$cat.name}</span>
                    <a class="hide" href="{categoryUrl category=$cat}"></a>
                </label>

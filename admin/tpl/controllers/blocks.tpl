...
{if $smarty.get.action}
    ...
    {include file='blocks'|cat:$smarty.const.RL_DS|cat:'m_block_start.tpl'}
        <form action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;block={$smarty.get.block}{/if}" method="post" enctype="multipart/form-data">
            ...
            {if $block_options}
                ...
                {if $option.type == 'hidden'}
                    <input type="hidden" value="{$option.default}" name="options[{$option_key}]" />
                    {continue}
                {/if}
                    ...
                    <td class="field">
                        ...
                        {elseif $option.type == 'select'}
                            <select name="options[{$option_key}]">
                                ...
                                <option value="{$value}" {if $value == $option.default}selected="selected"{/if}>
                                    {if $option_key == 'header_link_page_key'}
                                        {phrase key='pages+name+'|cat:$value}
                                    {else}
                                        {phrase key='blocks+option_value+'|cat:$value}
                                    {/if}
                                </option>
                                {/foreach}
                            </select>
                        ...
                        {elseif $option.type == 'text'}
                            <input type="text" value="{$option.default}" name="options[{$option_key}]" class="w350"{if $option.values} size="{$option.values}"{/if} />
                        {/if}
        ...
        {if $block_options && $block_options.header_link}
        <script type="text/javascript">
        {literal}

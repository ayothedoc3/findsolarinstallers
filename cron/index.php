...
if ($config['mail_method'] == 'smtp') {
    $rlMail->phpMailer->SMTPKeepAlive = true;
}
...
if (!$config['membership_module']
    || ($config['membership_module'] && $config['allow_listing_plans'])
) {
            ...
            if ($send_mail) {
                ...
                switch ($action) {
                    ...
                    case "pre_featured":
                        ...
                        $update_link = $reefless->getPageUrl('upgrade_listing', ['featured'], $listing['Lang'], "id={$listing['ID']}");
...
if ($config['mail_method'] == 'smtp' && $rlMail->phpMailer->SMTPKeepAlive) {
    $rlMail->phpMailer->SmtpClose();
}

...
class rlAjaxLang
{
    ...
    public function ajaxSetDefault($object, $code)
    {
        ...
        if ($GLOBALS['rlConfig']->setConfig('lang', $code)) {
            ...
            trigger_error("Can not set default language, MySQL problems", E_USER_WARNING);
    ...
    public function ajaxDeleteLang($id)
    {
        ...
        if (count($GLOBALS['languages']) == 2) {
            ...
            // Disable multilingual option for all fields
            $rlDb->query("UPDATE `{db_prefix}listing_fields` SET `Multilingual` = '0'");
            $rlDb->query("UPDATE `{db_prefix}account_fields` SET `Multilingual` = '0'");

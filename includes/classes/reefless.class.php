...
class reefless extends rlDb
{
    ...
    public function loadClass($className, $type = null, $plugin = null, $class_param = null)
    {
        ...
        if (!is_object($$className)) {
            ...
            if (defined('RL_CUSTOM') && file_exists(str_replace(RL_ROOT, RL_CUSTOM, $fileSource))) {
                require_once str_replace(RL_ROOT, RL_CUSTOM, $fileSource);
            } elseif (file_exists($fileSource)) {
    ...
    public function parsePhone($string = false, $field = false, bool $encode = true)
    {
        ...
        preg_match('/([^\|]+)?\|?(me:([0-9]+))?/', $string, $matches);

        if (!$matches) {
            return $string;
        }

        $out['number']     = $matches[1];
        $out['messengers'] = $matches[3];
        ...
        switch ($mode) {
            case 'listing':
                ...
                if ($config['mod_rewrite']) {
                    ...
                    $additionalPartUrl .= '-' . $listing['ID'];
                    $additionalPartUrl .= $config['html_in_pages'] ? '.html' : '/';
                ...
                if ($path || $data['key'] === 'home') {
                    ...
                    if ($listingTypeData
                        ...
                        if ($config['mod_rewrite']) {
                            ...
                            if ($path) {
                                $url .= $config['html_in_pages'] ? '.html' : '/';
                            }

                            $url .= $addVars;
                        } else {
                            $url .= $firstNonModrewriteSign . 'page=' . $path . $addUrl . $addVars;
                        }
                    }
                }
                break;
            ...
            case 'category':
                ...
                $catPostfix  = $config['html_in_categories'] ? '.html' : '/';

...
class rlCommon
{
        ...
        foreach ($data as $f2 => $v2) {
            ...
            if (false !== $poss) {
                switch ($fields[$poss]['Type']) {
                    ...
                    case 'phone':
                        $phone_error = null;
                        if ($fields[$poss]['Required'] && !$data[$f2]['number']) {
                            $phone_error = 'notice_field_empty';
                        } elseif ($data[$f2]['number'] && !$data[$f2]['valid']) {
                            $phone_error = 'notice_phone_field_error';
                        }

                        if ($phone_error) {
                            $errors[$step] = str_replace('{field}', '<span class="field_error">"' . $fields[$poss]['name'] . '"</span>', $lang[$phone_error]);

                            if ($admin) {
                                $error_fields[] = $prefix . "[{$fields[$poss]['Key']}][number]";
                            } else {
    ...
    public function metaRobots()
    {
        ...
        if ($page_info['Key'] == 'view_details'
            && $GLOBALS['listing_data']['Status'] == 'expired'
        ) {
    ...
    public function buildPagingUrlTemplate($add_url = '', $custom = '', $method = false, $var = false, $customSubdomain = false)
    {
        ...
        if ($config['mod_rewrite']) {
            ...
            if (is_numeric(strpos($add_url, ':')) || $custom) {
                ...
                $first_url .= $config['html_in_pages'] ? '.html' : '/';
            }

            $tpl_url .= '/index[pg]' . ($config['html_in_pages'] ? '.html' : '/');

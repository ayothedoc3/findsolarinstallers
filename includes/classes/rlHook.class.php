...
class rlHook
{
    ...
    public function load($name, &$param1 = null, &$param2 = null, &$param3 = null, &$param4 = null, &$param5 = null, &$param6 = null, &$param7 = null, &$param8 = null)
    {
        ...
        foreach ($rlHook->hooks[$name] as $entry) {
            ...
            try {
                ...
                // Run the code from the custom hooks
                elseif (isset($entry['callable'])) {
                    $entry['callable']($param1, $param2, $param3, $param4, $param5, $param6, $param7, $param8);
                }
                // legacy logic
                else {
     ...
     *
     * @param string   $name   - hook name
     * @param callable $content - PHP code or callable function
     *
     * @return void
     */
    public function addCustomHook($name, $content)
    {
        if (!$name || (!is_string($content) && !is_callable($content))) {
            return;
        }

        $index = is_callable($content) ? 'callable' : 'code';
        $this->hooks[$name][] = [$index => $content];
    }
}

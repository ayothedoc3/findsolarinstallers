...
class rlMessage
{
    ...
    public function ajaxRefreshMessagesArea($res_id = false, $checked = false, $visitor_mail = false, $admin = false)
    {
        ...
        if (empty($messages)) {
            if (defined('REALM') && REALM == 'admin') {
                ...
                $url = $reefless->getPageUrl('my_messages');
            }
    ...
    {
        global $_response, $config, $lang, $seller_info, $reefless;
        ...
        if ($config['messages_notification']) {
            ...
            $reply_link = $reefless->getPageUrl('my_messages');
    ...
    {
        global $config, $reefless, $rlMail, $rlListings;
        ...
        $listingTitle = $rlListings->getListingTitle(

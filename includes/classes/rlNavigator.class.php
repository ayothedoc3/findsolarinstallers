...
class rlNavigator
{
    ...
    public function definePage()
    {
        ...
        // System page request
        if ($pageInfo) {
            if (($pageInfo['Plugin'] && $pageInfo['Page_type'] == 'system'
                ...
                $pageInfo = $rlDb->getRow(
                    "SELECT * FROM `{db_prefix}pages`
                     WHERE `Key` = '404' AND `Status` = 'active' LIMIT 1"
                );
            }
    ...
    {
        // Domain validation removed — owner-operated single installation
        return;
    }
    ...
    public function transformLinks()
    {
        ...
        if ($ltypes_to_transform_links && $_GET['page']) {
            ...
            /* search results urls */
            $endOfUrl = $config['html_in_pages'] ? '.html' : '/';
            foreach ($ltypes_to_transform_links as $lk => $type_to_rewrite) {
                if ($type_to_rewrite['Links_type'] == 'subdomain') {
                    $ltype_on_sub = true;

                    if ($type_to_rewrite
                        && $type_to_rewrite['Links_type'] == 'subdomain'
                        && (
                            ($_GET['rlVareables'] == $search_results_url . $endOfUrl
                                || $_GET['rlVareables'] == $advanced_search_url . $endOfUrl
                            )

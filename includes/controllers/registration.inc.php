...
if (!$cur_step) {
    ...
    if ($_POST['step']) {
        ...
        if (!$errors) {
            ...
            Util::redirect($reefless->getPageUrl($page_info['Key'], [$next_step['path']]));
        }
    }
} else {
    switch ($cur_step) {
        ...
        case 'checkout':
            ...
            if (!$rlPayment->isPrepare()) {
                ...
                $success_url = $reefless->getPageUrl($page_info['Key'], ['step' => $next_step['path']]);
                $cancel_url  = $reefless->getPageUrl($page_info['Key'], ['step' => $get_step], null, 'canceled');
